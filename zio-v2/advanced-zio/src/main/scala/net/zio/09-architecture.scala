 ...
 *
 *  when is recommended to have Environments as part of your function, rather than explicit inputs to your service?
 *  - Local parameters put in the R
 *  - Global parameters put in the constructor
 */
package advancedzio.architecture

import advancedzio.architecture.comms.Email
import advancedzio.architecture.comms.Emailer
import advancedzio.architecture.orders.OrderRepo
import advancedzio.architecture.payments.PaymentProcessor
import zio._
...
package payments {
  ...
  import advancedzio.architecture.domain._
  ...
  import advancedzio.architecture.payments.Util._
...
package orders {
  ...
  final case class OrderRepoTest(ref: Ref[Map[Id, Order]]) extends OrderRepo {
    def update(id: Id, order: Order) : UIO[Unit] =
      ref.update(_.updated(id, order))
    def getOrderById(id: Id): IO[IOException, Option[Order]] =
