 ...
 * which are freely and safely interruption (unlike threads). The
 ...
 *
 * Yet, interruption can be one of the trickiest concepts to grasp in ZIO,
...
object Backpressuring extends ZIOSpecDefault {
  def spec =
    suite("Backpressuring") {
       ...
       * Finalizers cannot be interrupted
       */
      test("zipPar") {
        Live.live(for {
          start <- Clock.instant
         ...
         * Drawback
         * 1. Leak resources
         * 2. Real errors are gone b/c you get a fake exit value
         */
        test("disconnect") {
          Live.live(for {
            ref <- Ref.make(true)
    ...
    suite("BasicDerived") {
      ...
      test("ensuring") {
        ...
        def myOnExit[R, E, A](zio: ZIO[R, E, A])(finalizer: Exit[E, A] => UIO[Any]): ZIO[R, E, A] = {
          ZIO.checkInterruptible{ status: InterruptStatus =>
            ZIO.uninterruptible {
              (if (status.isInterruptible) zio.interruptible else zio).foldCauseZIO(
