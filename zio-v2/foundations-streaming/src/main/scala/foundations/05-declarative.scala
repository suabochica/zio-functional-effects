...
object Experiment {
  import zio._
  sealed trait ExperimentStreamTrait[+A] { self =>
    import foundations.declarative.Experiment.ExperimentStreamObject.{Empty, Concat, Ensuring, Make}

    final def ::[A1 >: A](a: => A1): ExperimentStreamTrait[A1] =
      ExperimentStreamObject.succeed(a) ++ self

    final def ++[A1 >: A](that: => ExperimentStreamTrait[A1]): ExperimentStreamTrait[A1] =
      ExperimentStreamObject.Concat(self, ExperimentStreamObject.suspend(that))


    final def foldZIO[S](s: S)(f: (S, A) => Task[S]): ZIO[Any, Throwable, S] = {
      def loop(self: ExperimentStreamTrait[A], s: S): ZIO[Scope, Throwable, S] =
        self match {
    ...
    final def flatMap[B](f: A => ExperimentStreamTrait[B]): ExperimentStreamTrait[B] =
      self match {
        case Empty => Empty
        case Make(make) => ExperimentStreamObject.unwrap(make.map(f))
        case Concat(left, right) => left.flatMap(f) ++ right.flatMap(f)
        case Ensuring(stream, finalizer) => Ensuring(stream.flatMap(f), finalizer)
      }

    final def filter(f: A => Boolean): ExperimentStreamTrait[A] =
      self.flatMap(a =>
        if (f(a)) ExperimentStreamObject(a)
        else ExperimentStreamObject.empty
      )
    final def flatten[B](implicit ev: A <:< ExperimentStreamTrait[B]): ExperimentStreamTrait[B] =
      self.flatMap(a => ev(a))

    final def drop(n: Int): ExperimentStreamTrait[A] =
      if (n <= 0) self
      else self.unconsToStreamWith(ExperimentStreamObject.empty, (_, t) => t.drop(n - 1))

    final def ensuring(finalizer: UIO[Any]): ExperimentStreamTrait[A] =
      ExperimentStreamObject.Ensuring(self, ExperimentStreamObject.unwrap(finalizer.as(ExperimentStreamObject.empty)))

    def unconsWithZIO[Z](empty: ZIO[Scope, Throwable, Z], cons: (A, ExperimentStreamTrait[A]) => ZIO[Scope, Throwable, Z]): ZIO[Scope, Throwable, Z] =
      uncons.flatMap {
        case None => empty
        case Some((head, tail)) => cons(head, tail)
      }
    def unconsWith[Z](empty: Z, cons: (A, ExperimentStreamTrait[A]) => Z): ZIO[Scope, Throwable, Z] =
      unconsWithZIO(ZIO.succeed(empty), (h, t) => ZIO.succeed(cons(h, t)))
    def unconsToStreamWith[B](empty: ExperimentStreamTrait[B], cons: (A, ExperimentStreamTrait[A]) => ExperimentStreamTrait[B]): ExperimentStreamTrait[B] =
      ExperimentStreamObject.unwrap(unconsWith(empty, cons))

    final def uncons: ZIO[Scope, Throwable, Option[(A, ExperimentStreamTrait[A])]] =
      self match {
        case Empty => ZIO.none
        case Make(make) => make.map(a => Some(a -> ExperimentStreamObject.empty))
        case Concat(left, right) =>
          left.uncons.flatMap {
            case None => right.uncons
            case Some((head, tail)) => ZIO.some(head -> ExperimentStreamObject.Concat(tail, right))
          }
  ...
  object ExperimentStreamObject {

    case object Empty extends ExperimentStreamTrait[Nothing]
    final case class Concat[+A](left: ExperimentStreamTrait[A], right: ExperimentStreamTrait[A]) extends ExperimentStreamTrait[A]
    final case class Ensuring[+A](stream: ExperimentStreamTrait[A], finalizer: ExperimentStreamTrait[Nothing]) extends ExperimentStreamTrait[A]
    final case class Make[+A](make: ZIO[Scope, Throwable, A]) extends ExperimentStreamTrait[A]

    val empty: ExperimentStreamTrait[Nothing] = Empty
    def apply[A](as: A*): ExperimentStreamTrait[A] =
      as.foldRight[ExperimentStreamTrait[A]](empty)(_ :: _)

    def unwrap[A](make: ZIO[Scope, Throwable, ExperimentStreamTrait[A]]): ExperimentStreamTrait[A] =
      Make(make).flatten

    def succeed[A](a: => A): ExperimentStreamTrait[A] =
      Make(ZIO.succeed(a))

    def suspend[A](stream: => ExperimentStreamTrait[A]): ExperimentStreamTrait[A] =
      Make(ZIO.attempt(stream)).flatten

    def fromZIO[A](zio: ZIO[Scope, Throwable, A]): ExperimentStreamTrait[A] =
      ExperimentStreamObject.unwrap(zio.map(ExperimentStreamObject(_)))
  }
}
