...
final private[checker] case class ReleaseCheckerLive(
  ...
  subscription: SubscriptionLogic.Service
  ...
  override def scheduleRefresh: Task[Unit] =
    for {
      ...
      latestVersions <- latestRepositoryVersions(repositories.keySet)
      updatedVersions = newVersions(repositories, latestVersions)
  ...
  private def getRepositoryStates(
    ...
    ZIO.foreach(updatedVersions.toList) { case (name, verions) =>
      subscriptionLogic
  ...
  ): IO[PageError, Map[Name, Option[Version]]] =
    ZIO
      .foreach(repositories) { name =>
        gitHubClient
          .releases(name)
          .map(
            releases => name -> Try(releases.maxBy(_.published_at).name),
            toOption
          )
      }
      .map(_.toMap)
